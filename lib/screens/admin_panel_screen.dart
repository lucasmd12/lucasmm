...
class _AdminPanelScreenState extends State<AdminPanelScreen> with TickerProviderStateMixin {
          ...
          child: ListTile(
            ...
            subtitle: Column(
              ...
              children: [
                ...
                Text(
                  "Clã: ${user.clanName ?? "N/A"} (Papel: ${user.clanRole.displayName})", // Corrigido para displayName
                  style: const TextStyle(color: Color(0xFFBDBDBD), fontSize: 12),
                ),
                Text(
            ...
            children: [
              Expanded(
                child: _buildReportCard(
                  "Usuários Ativos",
                  _allUsers.where((u) => u.online).length.toString(), // Alterado de u.isOnline para u.online
                  Icons.people,
                  Colors.green,
                ),
              ),
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: Role.values.map((role) => ListTile(
            title: Text(role.displayName, style: const TextStyle(color: Colors.white)), // Corrigido para displayName
            leading: Radio<Role>(
              value: role,
                ...
                if (value != null) {
                  try {
                    ...
                    await apiService.put(
                      ...
                      {
                        "role": roleToString(value), // Corrigido para roleToString
                      },
                    );
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text("Papel: ${user.role.displayName}", style: const TextStyle(color: Colors.white)), // Corrigido para displayName
  ...
  void _showResetPasswordDialog(User user) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text("Funcionalidade de resetar senha para ${user.username} ainda não implementada.")),
    );
  }
  ...
  void _showSuspendDialog(User user) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text("Funcionalidade de suspender usuário ${user.username} ainda não implementada.")),
    );
  }
