...
class _QRRDetailScreenState extends State<QRRDetailScreen> {
  ...
  Future<void> _updateQRRStatus(QRRStatus newStatus) async {
    try {
      setState(() => _isLoading = true);
      final qrrService = Provider.of<QRRService>(context, listen: false);
      await qrrService.updateQRRStatus(_qrr.id, newStatus.toString().split('.').last);
      await _refreshQRR();
        ...
        child: Column(
          ...
          children: [
            ...
            if (_qrr.participants.isEmpty)
              ...
              SizedBox(
                height: 100, // Altura fixa para a lista de avatares
                child: ListView.builder(
                  scrollDirection: Axis.horizontal,
                  itemCount: _qrr.participants.length > 5 ? 6 : _qrr.participants.length, // Mostrar até 5 avatares + 
