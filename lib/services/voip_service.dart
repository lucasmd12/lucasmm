...
import 'dart:async';
import 'package:voip_app/models/call_model.dart';
...
class VoipService with ChangeNotifier {
  ...
  bool get isInCall => _currentCall?.status == CallStatus.active;
  bool get isRinging => _currentCall?.status == CallStatus.ringing;
  bool get isCalling => _currentCall?.status == CallStatus.calling;
  ...
  Future<bool> initiateCall(String calleeId, String calleeUsername) async {
    ...
    try {
      ...
      if (response != null && response is Map<String, dynamic>) {
        // Update current call with actual ID from backend
        _currentCall = _currentCall!.copyWith(id: response['callId']);
  ...
  Future<List<Call>> getCallHistory() async {
    ...
    try {
      ...
      if (response != null && response is List) {
        ...
        return response.map((data) => Call.fromMap(data)).toList();
